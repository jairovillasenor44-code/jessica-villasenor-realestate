...
## Project
- **Pages:** `index.html` (home), `about.html`, `listings.html`, `contact.html`, `faq.html`, `process.html`, `home-search.html`, `valuation.html` — each self-contained with all CSS+JS inline (no shared stylesheet)
- **Server:** `node serve.mjs` → localhost:3000. Routes: `/` → `index.html`, `/about`, `/listings`, `/contact`, `/process`, `/home-search`, `/valuation` (cleanURLs), `/faq.html`, `/api/contact` → `api/contact.js`
...
## SEO / Analytics (all completed, live on all 8 pages)
- **GA4:** `G-D7P8XZKV6Q` — gtag snippet on every page before Google Fonts preconnect
- **Open Graph + Twitter Card** meta tags on all 8 pages; `og:image` uses absolute Vercel URL
- **Meta descriptions** on all 8 pages
- **`sitemap.xml`** — all 8 pages, `jessicavillasenor.com` domain, priorities set (/ = 1.0, etc.)
...
## Nav (all pages)
- `/about` · `/listings` (label: **Transactions**) · `/valuation` · `/home-search` · `/#testimonials` · `/contact` (Referrals) · `/faq.html`
- Brand logo → `/` on sub-pages, `#` on home
- Scroll threshold for solid background: `window.scrollY > 120`
- Sub-pages (`about`, `listings`, `contact`, `process`, `home-search`, `valuation`): nav brand is `<a href="/">` link
...
- **`home-search.html`**: 5 area cards (Reno, Sparks, Fernley, Carson City, Northern Lake Tahoe) linking to coombesgroupre.com search, plus map + "Talk to Jessica" CTA
- **`valuation.html`**: standalone Free Home Valuation page — split form panel, Why-It-Matters cards, FAQ accordion (4 items), Agent Trust Panel, footer
